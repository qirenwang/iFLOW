...
from Pareto_Multi_UCB_Algo import UCBCalculator, pareto_multi_ucb
...
class ModelManager:
    def __init__(self):
        ...
        log_file_path = "server_logs/training_history_pareto.txt"
...
def decide_model_for_next_training(performance_data):
    ...
    # 3. Call Pareto-Multi-UCB to select clients and models
...
if __name__ == "__main__":
    ...
    # Use specific data ratio values instead of random generation
    data_ratio = {
...
#             # Main thread decides the next model for each client
#             if self.client_id == 1:  # Only one thread should perform the decision
#                 # The choice of client_id == 1 is somewhat arbitrary; it could be any client ID. 
#                 # The key is to ensure that only one thread performs the decision-making. 
#                 # Using client_id == 1 is a simple way to consistently choose one thread across different runs.
#                 next_models = decide_model_for_next_training(self.model_manager.performance_data)
#                 self.model_manager.next_models = next_models
...
#     #     print('c, m, loss, current_round: ', c, m, loss, current_round)
#     #     ucb_calculator.update(c, m, loss, current_round)
